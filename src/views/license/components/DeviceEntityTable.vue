    ...
    <el-table-column label="设备参数" min-width="100" prop="deviceParamJson" type="expand">
      <template #default="props">
        <el-form-item label="设备参数：">
          <el-input v-model="props.row.deviceParamJson" :autosize="{ minRows: 1, maxRows: 100 }" :readonly="true"
                    type="textarea"/>
        </el-form-item>
      </template>
    </el-table-column>
...
function handleQuery() {
  ...
  listDeviceEntityPages(dataState.queryParams).then(({data}) => {
    data.list.forEach(value => {
      let item = value;
      item.deviceParamJson = JSON.stringify(item.deviceParam, null, 5);
