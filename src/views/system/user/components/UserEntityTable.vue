...
<template>
  ...
  >
    ...
    <el-table-column label="用户名称" min-width="200" prop="username"/>
    <el-table-column label="用户类型" min-width="150" prop="userType"/>
    <el-table-column label="用户角色" min-width="120" prop="role"/>
    <el-table-column label="用户权限" min-width="120" prop="permission"/>
    <el-table-column label="用户菜单" min-width="200" prop="menu"/>
...
function handleQuery() {
  ...
  listUserEntityPages(dataState.queryParams).then(({data}) => {
 ...
 * 刷新表单
 */
function refreshTable(queryParams: UserQueryParam) {
  dataState.queryParams = queryParams;
  if (dataState.queryParams.username === '') {
    dataState.queryParams.username = undefined;
  }
  if (dataState.queryParams.userType === '') {
    dataState.queryParams.userType = undefined;
  }
  if (dataState.queryParams.role === '') {
    dataState.queryParams.role = undefined;
  }
  if (dataState.queryParams.permission === '') {
    dataState.queryParams.permission = undefined;
  }
  if (dataState.queryParams.menu === '') {
