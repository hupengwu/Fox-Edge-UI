...
import RepositoryTemplateEntityTable from './components/RepositoryTemplateEntityTable.vue';
import RepositoryTemplateFormHeader from './components/RepositoryTemplateFormHeader.vue';
...
const repositoryTemplateEntityTableRef = ref<any>(); // 这了的变量名称必须和<deviceService-edit-dialog>的ref值一样
const repositoryTemplateFormHeaderRef = ref<any>(); // 这了的变量名称必须和<deviceService-edit-dialog>的ref值一样

const state = reactive({
  // 查询参数
  queryParams: {modelType: 'template', pageNum: 1, pageSize: 10} as RepositoryQueryParam,
});
...
function handleQuery(source: string, value: any) {
  ...
  repositoryTemplateEntityTableRef.value.refreshTable(queryParams);
}
...
function handleScan(source: string, value: any) {
  queryParams.value = value;
  repositoryTemplateEntityTableRef.value.scanService();
}
...
function getSelect(source: string, type: string, row: any) {
  ...
  if (source === 'header') {
    let rows = repositoryTemplateEntityTableRef.value.getSelects();
    if (rows.length === 0) {
      return;
    }
...
function handleDownload(source: string, type: string, row: any) {
  ...
  ElMessageBox.confirm('确认下载已选中的数据项的安裝包?', '警告', {
    ...
    downloadEntity(services).then(() => {
      ...
      // 刷新表单
      repositoryTemplateEntityTableRef.value.refreshTable(state.queryParams);
    });
  })
...
<template>
  <div class="app-container">
    ...
    <!-- 表单头：各种按钮 -->
    <repository-template-form-header ref="repositoryTemplateFormHeaderRef"
                                     @delete="handleDelete"
    ...
    <!-- 数据表单：创建/修改 -->
    <repository-template-entity-table ref="repositoryTemplateEntityTableRef"
                                      @delete="handleDelete"
                                      @download="handleDownload"
