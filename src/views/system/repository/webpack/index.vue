...
import RepositoryWebpackEntityTable from './components/RepositoryWebpackEntityTable.vue';
import RepositoryWebpackFormHeader from './components/RepositoryWebpackFormHeader.vue';
...
const repositoryWebpackEntityTableRef = ref<any>(); // 这了的变量名称必须和<deviceService-edit-dialog>的ref值一样
const repositoryWebpackFormHeaderRef = ref<any>(); // 这了的变量名称必须和<deviceService-edit-dialog>的ref值一样
...
function handleQuery(source: string, value: any) {
  ...
  repositoryWebpackEntityTableRef.value.refreshTable(queryParams);
}
...
function handleScan(source: string, value: any) {
  queryParams.value = value;
  repositoryWebpackEntityTableRef.value.scanService();
}
...
function getSelect(source: string, type: string, row: any) {
  ...
  if (source === 'header') {
    let rows = repositoryWebpackEntityTableRef.value.getSelects();
    if (rows.length === 0) {
      return;
    }
...
function handleDownload(source: string, type: string, row: any) {
  ...
  ElMessageBox.confirm('确认下载已选中的数据项的安裝包?', '警告', {
    ...
    downloadEntity(services).then(() => {
      ...
      // 刷新表单
      repositoryWebpackEntityTableRef.value.refreshTable(state.queryParams);
    });
  })
...
<template>
  <div class="app-container">
    ...
    <!-- 表单头：各种按钮 -->
    <repository-webpack-form-header ref="repositoryWebpackFormHeaderRef"
                                    @delete="handleDelete"
    ...
    <!-- 数据表单：创建/修改 -->
    <repository-webpack-entity-table ref="repositoryWebpackEntityTableRef"
                                     @delete="handleDelete"
                                     @download="handleDownload"
