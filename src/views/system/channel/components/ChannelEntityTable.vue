...
<template>
  ...
  >
    ...
    <el-table-column label="通道名称" min-width="120" prop="channelName"/>
    <el-table-column label="通道类型" min-width="120" prop="channelType"/>
    <el-table-column label="通道参数" min-width="120" prop="channelParam" type="expand">
      <template #default="props">
        <el-input v-model="props.row.channelParamJson" :autosize="{ minRows: 1, maxRows: 100 }" :readonly="true"
                  type="textarea"/>
      </template>
    </el-table-column>
...
function handleQuery() {
  ...
  listChannelEntityPages(dataState.queryParams).then(({data}) => {
    data.list.forEach(value => {
      let item = value;
      item.channelParamJson = JSON.stringify(value.channelParam, null, 5);
 ...
 * 刷新表单
 */
function refreshTable(queryParams: ChannelQueryParam) {
