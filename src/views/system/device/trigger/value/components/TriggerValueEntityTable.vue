...
<template>
  ...
  >
    ...
    <el-table-column label="触发器名称" min-width="120" prop="triggerConfigName"/>
...
import {
  listTriggerValueEntityPages,
} from "@/api/device/trigger/value";
...
let dataState = reactive({
  ...
  // 实体数据
  entityList: [] as TriggerValueItem[],
});
...
function handleQuery() {
  ...
  listTriggerValueEntityPages(dataState.queryParams).then(({data}) => {
 ...
 * 刷新表单
 */
function refreshTable(queryParams: TriggerValueQueryParam) {
