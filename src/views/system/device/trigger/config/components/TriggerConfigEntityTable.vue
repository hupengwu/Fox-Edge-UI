...
<template>
  ...
  >
    ...
    <el-table-column label="配置名称" min-width="120" prop="triggerConfigName"/>
    <el-table-column label="对象范围" min-width="120" prop="objectRange"/>
    <el-table-column label="设备名称" min-width="120" prop="deviceName"/>
    <el-table-column label="设备类型" min-width="120" prop="deviceType"/>
    <el-table-column label="配置信息" min-width="120" prop="objectList" type="expand">
      <template #default="props">
        <el-form-item label="对象列表">
          <el-input v-model="props.row.objectListJson" :autosize="{ minRows: 1, maxRows: 100 }" :readonly="true"
                    type="textarea"/>
        </el-form-item>
        <el-form-item label="参数配置">
          <el-input v-model="props.row.paramsJson" :autosize="{ minRows: 1, maxRows: 100 }" :readonly="true"
                    type="textarea"/>
        </el-form-item>
      </template>
    </el-table-column>
    <el-table-column label="模块名称" min-width="120" prop="triggerModelName"/>
    <el-table-column label="方法名称" min-width="120" prop="triggerMethodName"/>
    <el-table-column label="队列深度" min-width="120" prop="queueDeep"/>
...
let dataState = reactive({
  ...
  // 实体数据
  entityList: [] as TriggerConfigItem[],
});
...
function handleQuery() {
  ...
  listTriggerConfigEntityPages(dataState.queryParams).then(({data}) => {
    data.list.forEach(value => {
      let item = value;
      item.objectListJson = JSON.stringify(value.objectList, null, 5);
      item.paramsJson = JSON.stringify(value.params, null, 5);
