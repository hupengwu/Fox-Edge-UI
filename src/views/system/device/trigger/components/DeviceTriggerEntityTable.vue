...
<template>
  ...
  >
    ...
    <el-table-column label="方法类型" min-width="120" prop="methodName"/>
    <el-table-column label="配置类型" min-width="120" prop="manufacturer"/>
...
import {
  listDeviceTriggerEntityPages,
} from "@/api/device/trigger";
...
let dataState = reactive({
  ...
  // 实体数据
  entityList: [] as DeviceTriggerItem[],
});
...
function handleQuery() {
  ...
  listDeviceTriggerEntityPages(dataState.queryParams).then(({data}) => {
