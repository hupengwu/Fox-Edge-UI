...
<template>
  ...
  >
    ...
    <el-table-column label="设备名称" min-width="160" prop="deviceName"/>
    <el-table-column label="设备类型" min-width="120" prop="deviceType"/>
    <el-table-column label="设备参数" min-width="100" prop="deviceParamJson" type="expand">
      <template #default="props">
        <el-form-item label="设备参数：">
          <el-input v-model="props.row.requestJson" :autosize="{ minRows: 1, maxRows: 100 }" :readonly="true"
                    type="textarea"/>
        </el-form-item>
      </template>
    </el-table-column>
    <el-table-column label="重试次数" min-width="100" prop="commFailedCount"/>
    <el-table-column :formatter="formatDate4elTableColumn" align="center" label="失败时间" prop="commFailedTime"
                     width="180"/>
    <el-table-column :formatter="formatDate4elTableColumn" align="center" label="成功时间" prop="commSuccessTime"
                     width="180"/>
...
import {
  listDeviceTimeOutEntityPages,
} from "@/api/device/timeout";
...
let dataState = reactive({
  ...
  // 实体数据
  entityList: [] as DeviceTimeOutItem[],
});
...
function handleQuery() {
  ...
  listDeviceTimeOutEntityPages(dataState.queryParams).then(({data}) => {
    data.list.forEach(value => {
      let item = value;
      item.requestJson = JSON.stringify(item.request, null, 5);
 ...
 * 刷新表单
 */
function refreshTable(queryParams: DeviceTimeOutQueryParam) {
