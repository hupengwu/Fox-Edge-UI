<script lang="ts">
export default {
  name: "OperateManualTaskEntityTable"
}
</script>
...
<template>
  ...
  >
    ...
    <el-table-column label="设备名称" min-width="250" prop="deviceName"/>
...
import {
  listOperateManualTaskEntityPages,
...
let dataState = reactive({
  ...
  // 实体数据
  entityList: [] as OperateManualTaskItem[],
});
...
function handleQuery() {
  ...
  listOperateManualTaskEntityPages(dataState.queryParams).then(({data}) => {
 ...
 * 刷新表单
 */
function refreshTable(queryParams: OperateManualTaskQueryParam) {
