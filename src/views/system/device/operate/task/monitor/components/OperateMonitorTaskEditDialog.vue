<script lang="ts">
export default {
  name: "OperateMonitorTaskEditDialog"
}
</script>
...
let dataState = reactive({
  ...
  formData: {
    ...
    templateParamJson: '[]',
    deviceIds: [] as number[],
  } as CreateOperateMonitorTaskRequestVO,
...
function handleSubmit() {
  if (!formData.value.templateParamJson?.startsWith('[') || !formData.value.templateParamJson?.endsWith(']')) {
    ElMessage.info('配置参数的格式，不是合法的JSON数组格式!');
    return;
  }
  ...
  if (type.value === 'create') {
    ...
    formData.value.templateParamJson = undefined;
