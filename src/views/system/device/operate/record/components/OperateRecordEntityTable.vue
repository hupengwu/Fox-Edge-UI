...
<template>
  ...
  >
    ...
    <el-table-column label="事件名称" min-width="120" prop="recordName"/>
    <el-table-column label="事件数据" min-width="120" prop="recordData" type="expand">
      <template #default="props">
        <el-form-item label="操作参数">
          <el-input v-model="props.row.recordParamJson" :autosize="{ minRows: 1, maxRows: 100 }" :readonly="true"
                    type="textarea"/>
        </el-form-item>
        <el-form-item label="操作结果">
          <el-input v-model="props.row.recordDataJson" :autosize="{ minRows: 1, maxRows: 100 }" :readonly="true"
                    type="textarea"/>
        </el-form-item>
      </template>
    </el-table-column>
...
import {
  listOperateRecordEntityPages,
} from "@/api/device/operate/record";
...
let dataState = reactive({
  ...
  // 实体数据
  entityList: [] as OperateRecordItem[],
});
...
function handleQuery() {
  ...
  listOperateRecordEntityPages(dataState.queryParams).then(({data}) => {
    data.list.forEach(value => {
      let item = value;
      item.recordDataJson = JSON.stringify(value.recordData, null, 5);
      item.recordParamJson = JSON.stringify(value.recordParam, null, 5);
 ...
 * 刷新表单
 */
function refreshTable(queryParams: OperateRecordQueryParam) {
