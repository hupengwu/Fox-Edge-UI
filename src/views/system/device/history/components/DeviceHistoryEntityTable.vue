...
<template>
  ...
  >
    ...
    <el-table-column label="对象ID" min-width="120" prop="objectName"/>
    <el-table-column label="参数数值" min-width="120" prop="paramValue"/>
...
import {
  listDeviceHistoryEntityPages,
} from "@/api/device/history";
...
let dataState = reactive({
  ...
  // 实体数据
  entityList: [] as DeviceHistoryItem[],
});
...
function handleQuery() {
  ...
  listDeviceHistoryEntityPages(dataState.queryParams).then(({data}) => {
 ...
 * 刷新表单
 */
function refreshTable(queryParams: DeviceHistoryQueryParam) {
  dataState.queryParams = queryParams;
  if (dataState.queryParams.paramValue === '') {
