...
<template>
  ...
  >
    ...
    <el-table-column label="业务名称" min-width="200" prop="serviceName"/>
    <el-table-column label="配置名称" min-width="150" prop="configName"/>
    <el-table-column label="配置参数" min-width="80" prop="configValue" type="expand">
      <template #default="props">
        <el-form-item label="描述信息">
          <el-input v-model="props.row.remark" :autosize="{ minRows: 1, maxRows: 100 }" :readonly="true"
                    type="textarea"/>
        </el-form-item>
        <el-form-item label="配置参数">
          <el-input v-model="props.row.configValueJson" :autosize="{ minRows: 1, maxRows: 100 }" :readonly="true"
                    type="textarea"/>
        </el-form-item>
      </template>
    </el-table-column>
...
function handleQuery() {
  ...
  listConfigEntityPages(dataState.queryParams).then(({data}) => {
    data.list.forEach(value => {
      let item = value;
      item.configValueJson = JSON.stringify(value.configValue, null, 5);
 ...
 * 刷新表单
 */
function refreshTable(queryParams: ConfigQueryParam) {
  ...
  if (dataState.queryParams.serviceId === '') {
    dataState.queryParams.serviceId = undefined;
  }
  if (dataState.queryParams.configName === '') {
