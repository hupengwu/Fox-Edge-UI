...
<template>
  ...
  >
    ...
    <el-table-column label="绑定设备" min-width="80" prop="countDeviceBind"/>
    <el-table-column label="可绑设备" min-width="80" prop="countDeviceType"/>
    <el-table-column label="关联对象" min-width="80" prop="countObjectBind"/>
...
function handleBindDialog(row: any) {
  emit('binding', 'table', 'device', row.id);
}
...
function handleQuery() {
  ...
  listPeriodTaskEntityPages(dataState.queryParams).then(({data}) => {
    data.list.forEach(value => {
      let item = value;
      item.taskParamJson = JSON.stringify(value.taskParam, null, 5);
 ...
 * 刷新表单
 */
function refreshTable(queryParams: PeriodTaskQueryParam) {
